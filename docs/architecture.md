...
Every response receives a generated request ID and browser security headers. Health, chat, and scenario POST responses are never cached; historical GET APIs use a five-minute cache; fingerprinted static assets are immutable for one year; SPA shells revalidate. Large responses use gzip. Prediction, scenario, and review completion logs contain only the request ID, race ID, model version, operation kind, elapsed time, driver count, probability sum, or hit count. The append-only `scripts.verify_public_release` command challenges request-ID reflection and verifies these delivery rules plus the SPA, archive, readiness, released model, bounded grid scenario, review, and model-card contracts from the public origin. Its optional single typo-correction chat probe writes only allowlisted counts and status fields.

The verified Grid Scenario application release is Git SHA `7738da72311a390509d92377ef11b24d8e95a0e1`, CodeBuild `f1-race-strategist-dev:a1c49bad-70f5-4a74-a080-8ec21fdee065`, ECR digest `sha256:49f9d2697fd0ea38cfd5563c4238c6e3263f20cbbbe32594ff28a79413fbc2fd`, and Lightsail deployment 17 in `eu-north-1`. Deployment 16 performed and verified a private reader-password rotation; deployment 17 contains only the application container. Continuous stdout/stderr remains in Lightsail. The operator export bounds the active deployment window, follows every returned page up to a safety limit, and writes a chronological snapshot to the 14-day CloudWatch group `/f1/dev/api`. Verified stream `lightsail/20260911T132027240680Z` contains prediction, scenario, review, and chat completion records; configured credentials, question text, SQL, prompts, and connection strings were absent from readback. The public `reports/grid_scenario_aws_release_v17.json` and combined `reports/grid_scenario_cloud_verification.json` bind these checks to the deployed SHA; the deployment-14 report remains unchanged as historical evidence.

## Deliberate limits

The archive has no dependable weather, tyre, fuel or telemetry data and ends in 2024. The race-level projection constrains marginal probabilities; it is not a joint finishing-order distribution. Grid scenarios are counterfactual model sensitivity checks and do not establish causal track effects or new validation evidence. The public demo uses an access code and per-process rate/concurrency limits rather than individual accounts. The quota resets when a container restarts. The small service and database have no multi-region failover, durable distributed quota or notification subscriber. See `docs/deployment.md` for rollback, retained AWS resources and costs.
