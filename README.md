...
The verified dev application runs at [the AWS Lightsail HTTPS endpoint](https://f1-strategist-demo.ys85rp5g9ncdj.eu-north-1.cs.amazonlightsail.com/), backed by private encrypted PostgreSQL. Lightsail deployment 9 runs immutable image `4b3946451986f09fdd11d880a2de757366947989`; public checks cover frontend deep links, health, comparison, profiles, predictions, spelling-aware statistics with visible SQL and trace, structured follow-up, ambiguity clarification, unsupported-data refusal and selected-race explanation. Runtime startup checks its database credentials are SELECT-only. GitHub Actions run `34456776706` and CodeBuild `cce52796-d309-4d3c-a536-4a97e796c0a1` passed. Safe completion records were exported to CloudWatch after checking that credentials, access codes, request text and SQL were absent; continuous application logs remain in Lightsail.
