...
The verified dev application runs at [the AWS Lightsail HTTPS endpoint](https://f1-strategist-demo.ys85rp5g9ncdj.eu-north-1.cs.amazonlightsail.com/), backed by private encrypted PostgreSQL. The deployed image is commit `8d0e8d1`; public checks cover frontend deep links, health, comparison, profiles, predictions, spelling-aware statistics with visible SQL, ambiguity clarification and unsupported-data refusal. Runtime startup checks its database credentials are SELECT-only. CodeBuild and runtime snapshots are in CloudWatch, while continuous application logs remain in Lightsail.
