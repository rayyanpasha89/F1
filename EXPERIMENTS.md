...
## Podium Outcome Lab — derived mapping, not a new experiment

Executed `python -m scripts.evaluate_podium_outcomes` after the marginal probability model and podium-count projection were already fixed. No new model was trained or selected. For each supported race, the service enumerates every unordered three-driver subset and solves the maximum-entropy distribution whose membership marginals match the released driver probabilities. It uses no race result, podium label, status, point, lap, or pit-stop value.

Across the 68 supported races, all 77,349 subset probabilities converged. Per-race mass summed to one, reconstructed membership counts summed to three within `8.89e-16`, and the maximum single-driver reconstruction error was `3.82e-13`. The aggregate structural and latency evidence is in `reports/podium_outcome_local_evaluation.json`.

These checks establish numerical coherence and bounded runtime for this deterministic mapping. They do not measure ordered finishing accuracy, learn correlations between drivers, establish causal interaction, or create a fresh joint-outcome validation set. Any later claim that the set probabilities improve decision quality needs a prospectively declared evaluation separate from the consumed 2022–2024 outcomes.
